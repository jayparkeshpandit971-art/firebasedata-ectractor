...
/**
 * ============================================================
 * LISA'S FIREBASE DUMPER BOT - RENDER EDITION
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    // Welcome page
    echo "<h1>🔥 Firebase Dumper Bot</h1>";
    echo "<p>Bot is running! Set webhook: <a href='?set_webhook'>Click here</a></p>";
    exit;
}
...
function handleStatus($chatId) {
    sendMessage($chatId,
        ...
        "🔓 Bypass: ENABLED\n" .
        "🚀 Methods: " . count($GLOBALS['bypassSettings']['bypassMethods']) . " active"
    );
}
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && ($_SERVER['REQUEST_URI'] === '/health' || $_SERVER['REQUEST_URI'] === '/')) {
    header('Content-Type: application/json');
    echo json_encode([
        'status' => 'healthy',
        'bot' => 'Firebase Dumper Bot',
        'time' => date('Y-m-d H:i:s'),
        'webhook' => 'https://' . $_SERVER['HTTP_HOST'] . '/bot.php'
    ]);
    exit;
}
?>
